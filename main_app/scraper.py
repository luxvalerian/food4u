        ...
        if alive:
            ...
            for idx in UNITS:
                if idx[0] == unit and unit != None:
                    unit = idx[1]
            product_list.append({'img_url': food['basic']['image']['thumbnail'], 'name': food['basic']['name'][:100], 'price': food['store']['price']['list'], 'unit': unit[:1], 'counts': food['store']['price']['salesQuantity']})
    piece = ''
    for item in product_list:
        ...
        if piece == item['img_url']:
            ...
            pass
              ...
              # print(item['name'])
              item = Item(name=item['name'], unit_price=item['price'], item_count=item['counts'], description=description, unit_measurement=item['unit'], image=item['img_url'], store=stores.first())
              item.save()
